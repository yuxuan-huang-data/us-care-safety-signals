...
**Facility-level link to pressure injury.** Facilities with higher resident pressure-ulcer rates (CMS) do not have higher staff injury rates (OSHA). Across 7,370 facilities linked through the [`crosswalk`](../crosswalk/), the DART injury rate and the long-stay pressure-ulcer rate are uncorrelated (Pearson r = −0.001; Spearman ρ = 0.024). An earlier, narrower linkage gave r = 0.011 (n = 3,497). The test and the linkage code are published in the crosswalk module.

The likely explanation is measurement: a few injury cases per facility per year is too few to separate real differences from differences in reporting. The null result concerns facility-level correlation only.
